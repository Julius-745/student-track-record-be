...
const generatePelaporan = async (count: number, siswaList: Siswa[], guruList: Guru[]) => {
    ...
    for(let i=0; i < count; i++) {
        ...
        pelaporan.jenis_pelaporan = faker.helpers.arrayElement(['prestasi', 'pelanggaran']);
        
        if (pelaporan.jenis_pelaporan === 'prestasi') {
             pelaporan.deskripsi = faker.helpers.arrayElement([
                 'Juara 1 Lomba Matematika', 'Juara 2 Lomba Puisi', 'Membantu guru membersihkan kelas', 'Menjadi petugas upacara terbaik', 'Juara Kelas'
             ]);
        } else {
             pelaporan.deskripsi = faker.helpers.arrayElement([
                 'Terlambat masuk sekolah', 'Tidak mengerjakan PR', 'Ribut di kelas', 'Membuang sampah sembarangan', 'Tidak memakai atribut lengkap'
             ]);
        }
        
        pelaporan.tanggal = faker.date.recent({ days: 30 }).toISOString().split('T')[0];
...
const seed = async () => {
    try {
        ...
        await AppDataSource.query('TRUNCATE TABLE "pelaporan", "siswa", "guru" RESTART IDENTITY CASCADE;');
